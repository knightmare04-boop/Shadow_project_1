...
class GraphModuleSerializableCallable(SerializableCallable):
    def __init__(self, graph_module: torch.fx.GraphModule) -> None:
        if not isinstance(graph_module, torch.fx.GraphModule):
            raise AssertionError(
                f"Expected torch.fx.GraphModule, got {type(graph_module)}"
            )
    ...
    def deserialize_compile_artifacts(cls, data: bytes) -> Any:
        ...
        if not isinstance(state["graph_module"], torch.fx.GraphModule):
            raise AssertionError(
                f"Expected deserialized graph_module to be torch.fx.GraphModule, "
                f"got {type(state['graph_module'])}"
            )
...
class BundledAOTAutogradSerializableCallable(SerializableCallable):
    ...
    def __init__(self, compiled_fn: Any) -> None:
        ...
        if not hasattr(compiled_fn, "serialize"):
            raise AssertionError(
                f"compiled_fn must have a 'serialize' method, got {type(compiled_fn)}"
            )
